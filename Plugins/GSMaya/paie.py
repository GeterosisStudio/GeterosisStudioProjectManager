...
class PaieGUI:
	...
	def makeTab(self, index ):
		...
		if self.mode == 1: #Import
			...
			#mc.button(label = "Import", align = "center", command = Callback(self.importButton) ) # Using mel wrapper callback to fix undo bug in previous GSMaya versions
	...
	def refreshGUI(self):
		# This is crucial for not to crash GSMaya when switching import/export mode
	...
	# Deprecated. Used in relation to fileBrowserDialog as a replacement to fileDialog2 which didn't work previously
		if platformCase == "ms":
			...
			print "# PaieGUI.browseFolders >> GSMaya version: ", version
