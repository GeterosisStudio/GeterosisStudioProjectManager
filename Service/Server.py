...
class Server:
    ...
    def __init__(self):
        self.meassages = []
        self.lock_meassages = threading.RLock()
    ...
    def handle_client(self, client_socket, client_address):
        ...
        while self.server_status:
            try:
                ...
                if not data:
                    break
                self.lock_meassages.acquire()
                self.meassages.append(data)
                # Отправка ответа клиенту
                client_socket.sendall("Сообщение получено 88".encode('utf-8'))
                self.lock_meassages.release()
