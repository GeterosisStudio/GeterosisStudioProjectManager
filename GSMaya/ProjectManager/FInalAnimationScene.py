"""Finaling animation scene to unreal"""
...
def work_incremental_save(path=None):
    if not path:
...
def get_chars():
    chars = list_all_children("chars_grp")
    chars_names = []
...
def check_sequence_correct():
    ...
    for shot in shots:
        ...
        sequence_start_frame = cmds.getAttr("{}.sequenceStartFrame".format(shot))  # Query shot's sequence start frame.
...
def export_cameras():
    cam_export_path = get_export_path() + "cameras/"
    shots = cmds.ls(type="shot")
...
def export_body(root_name):
    ...
    char_export_path = get_export_path() + "chars/"
...
def export_face(root_name):
    namespace = root_name.split(":")[0]
    face_export_path = get_export_path() + "chars/"
    face_export_name = face_export_path + str(namespace) + "_face" + ".fbx"
    face_ref = [namespace + ":DHI_rig"]
    face_ref_geos = list_all_children(namespace + ":FacialControls")
    ...
    bake_face = [u'CTRL_L_mouth_tightenD', u'CTRL_L_mouth_tightenU', u'CTRL_R_mouth_tightenU', u'CTRL_R_mouth_tightenD',
                 ...
                 u'CTRL_R_jaw_ChinRaiseD', u'CTRL_L_jaw_ChinRaiseD', u'CTRL_L_jaw_clench', u'CTRL_R_jaw_chinCompress',
                 ...
                 u'CTRL_L_mouth_towardsU', u'CTRL_R_mouth_funnelU', u'CTRL_R_mouth_towardsD', u'CTRL_L_mouth_funnelU',
                 ...
                 u'CTRL_C_teeth_fwdBackD', u'CTRL_C_mouth_lipShiftD', u'CTRL_convergenceSwitch', u'CTRL_R_brow_down',
                 ...
                 u'CTRL_R_eye_cheekRaise', u'CTRL_C_eye_parallelLook', u'CTRL_L_eye_lidPress', u'CTRL_R_eye_lidPress',
...
def create_export_config():
    export_config_path = get_export_path() + "config.json"
    sets = "sets"
    scene = {}
    shots_arr = {}
    chars_arr = {}
    ...
    for i in range(len(chars)):
        if cmds.objExists(sets):
            offset_x = cmds.getAttr("{}:rig:World.translateX".format(chars[i])) + (cmds.getAttr("{}.translateX".format(sets)) * -1)
            offset_y = cmds.getAttr("{}:rig:World.translateY".format(chars[i])) + (cmds.getAttr("{}.translateY".format(sets)) * -1)
            offset_z = cmds.getAttr("{}:rig:World.translateZ".format(chars[i])) + (cmds.getAttr("{}.translateZ".format(sets)) * -1)
        else:
