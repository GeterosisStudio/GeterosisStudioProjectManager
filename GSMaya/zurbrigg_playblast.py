...
class ZurbriggPlayblast(QtCore.QObject):
    ...
    VIEWPORT_VISIBILITY_PRESETS = {
        ...
        "Chars": ["NURBS Surfaces", "Polygons", "NURBS Curves"],
    }
    ...
    def get_sequence_start_time(self, shots):
        start_shots = cmds.getAttr(shots[0] + ".startFrame")
        end_shots = cmds.getAttr(shots[0] + ".endFrame")
        for i in range(len(shots)):
            if start_shots > cmds.getAttr(shots[i] + ".startFrame"):
                start_shots = cmds.getAttr(shots[i] + ".startFrame")
        return start_shots


    def get_sequence_end_time(self, shots):
        start_shots = cmds.getAttr(shots[0] + ".startFrame")
        end_shots = cmds.getAttr(shots[0] + ".endFrame")
        for i in range(len(shots)):
            if end_shots < cmds.getAttr(shots[i] + ".endFrame"):
                end_shots = cmds.getAttr(shots[i] + ".endFrame")
        return end_shots


    def execute(self, output_dir, filename, padding=4, overscan=False, show_ornaments=True, show_in_viewer=True, overwrite=False, shots=False):
        ...
        if not shots:
            ...
            options = {
                ...
                "widthHeight": widthHeight,
                "percent": 100,
                "startTime": self.get_sequence_start_time(shots),
                "endTime": self.get_sequence_end_time(shots),
                "sequenceTime": 1,
                "clearCache": True,
                "forceOverwrite": force_overwrite,
                "format": "image",
    ...
    def set_single_camera_panel(self, camera_name):
        gMainPane = mel.eval('global string $gMainPane; $temp = $gMainPane;')
        cmds.paneLayout(gMainPane, configuration='single', e=True)
    ...
    def resolve_output_directory_path(self, dir_path):
        if "{project}" in dir_path:
            dir_path = dir_path.replace("{project}", self.get_file_dir_path())
    ...
    def get_file_dir_path(self):
        work_dir = cmds.file(q=True, sceneName=True).replace(cmds.file(q=True, sceneName=True, shortName=True), "")
        return work_dir.replace("/work", "")
...
class ZurbriggPlayblastUi(QtWidgets.QDialog):
    ...
    def create_widgets(self):
        ...
        self.output_dir_path_le.setPlaceholderText("{project}")
        ...
        self.playblast_sq_btn = QtWidgets.QPushButton("Playblast SQ")
    ...
    def create_layout(self):
        ...
        button_layout.addWidget(self.playblast_sq_btn)
    ...
    def create_connections(self):
        ...
        self.playblast_sq_btn.clicked.connect(self.do_playblast_sq)
    ...
    def do_playblast_sq(self):
        shots = cmds.ls(type="shot", selection=True)
        if not shots:
        ...
        self._playblast.execute(output_dir_path, filename, padding, overscan, show_ornaments, show_in_viewer, overwrite, shots)
    ...
    def select_output_directory(self):
        ...
        if not file_info.exists():
            current_dir_path = self._playblast.get_file_dir_path()
