...
def set_animation_retarget_source_asset(animation_asset_path, retarget_source_path):
    animation_asset = unreal.EditorAssetLibrary.load_asset(animation_asset_path)
    retarget_source = unreal.EditorAssetLibrary.load_asset(retarget_source_path)
    animation_asset.set_retarget_source(retarget_source)
    animation_asset.save_asset()
...
def buildAnimationImportOptions(
        skeleton_path='/Game/Mannequin/Character/Mesh/UE4_Mannequin_Skeleton.UE4_Mannequin_Skeleton'):
    options = unreal.FbxImportUI()
    # unreal.FbxImportUI
...
# Get full path from sequencer, create and it
def create_sequence(sequence_path_and_name):
    sequence_name = sequence_path_and_name.split("/")[-1]
    sequence_path = sequence_path_and_name.replace(sequence_name, "")

    unreal.AssetToolsHelpers.get_asset_tools().create_asset(sequence_name, sequence_path, unreal.LevelSequence,
                                                            unreal.LevelSequenceFactoryNew())
    unreal.EditorAssetLibrary.save_asset(sequence_path_and_name, True)


def import_asset_in_sequence(sequence_path, asset_path):
    # Получить ссылку на Level Sequence
    level_sequence = unreal.LevelSequence.cast(unreal.EditorAssetLibrary.load_asset(sequence_path))

    # Загрузить ассет
    asset = unreal.load_asset(asset_path)

    # Найти Spawnable по имени
    spawnable_name = asset.get_name()

    # Если Spawnable не найден, добавить его в Level Sequence
    spawnable = level_sequence.add_spawnable_from_instance(asset)

    # Сохранить изменения в Level Sequence
    unreal.EditorAssetLibrary.save_loaded_asset(level_sequence)



def add_folder_in_sequence(name, sequence_path):
    sequence = unreal.load_asset(sequence_path)
    sequence.add_root_folder_to_sequence(name)


def add_audio_track_in_sequence(sequence_path, name):
    level = unreal.load_asset(sequence_path)
    obj = unreal.MovieSceneAudioTrack()
    audio = level.add_master_track(type(obj))
    audio.set_editor_property("display_name", name)
